...
if __name__ == "__main__":
    ...
    for label_name in os.listdir(label_path):
        # Setup image path and read in image
        img = cv2.imread('F:/VOCtrainval_images/' + label_name[0:-4] + '.jpg')
        height, width = img.shape[0], img.shape[1]
        
        # Read in coors in txt form label, [xcen, ycen, w, h] to [xmin, ymin, xmax, ymax]
        # res = xywh_to_xyxy(txt_target(label_path + label_name, height, width))
        
        # Read in coors in xml form label
        ...
        for i in range(res.shape[0]):
            for j in range(len(cutinfo[i])):
                ...
                cv2.imwrite(save_path + str(label_name) + '_' + str(i) + '_' + str(j) + '.jpg', img_temp)
